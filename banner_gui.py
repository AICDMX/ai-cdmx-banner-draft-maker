...
if "SpeakerPhoto" in layers:
    ...
    try:
        photo_image = photo_result.index(0).get_image() if photo_result else None
    except (IndexError, AttributeError):
        photo_image = None
...
file_obj = Gio.File.new_for_path("{escape(template_path)}")
config.set_property("file", file_obj)
print(f"DEBUG: Loading template from {{'{escape(template_path)}'}}")
result = proc.run(config)
print(f"DEBUG: Template load result type: {{type(result)}}")
if hasattr(result, 'length'):
    print(f"DEBUG: Result length: {{result.length()}}")
try:
    # In GIMP 3.0, result is a ValueArray with status at index 0 and image at index 1
    image = None
    if hasattr(result, 'index'):
        try:
            ...
            val = result.index(1)
            print(f"DEBUG: Index 1 type: {{type(val)}}, value: {{val}}")
            # Index 1 might be the Image directly or wrapped
            ...
            print(f"DEBUG: Failed to load image: {{load_err}}")
            image = None
    else:
        image = result if hasattr(result, 'list_layers') else None
except (IndexError, AttributeError) as ex:
    print(f"DEBUG: Exception loading image: {{ex}}")
    image = None
...
# Save as XCF (with layers intact) using PDB procedure
try:
    # Try gimp-image-save-as (newer GIMP 3.0 native method)
    proc = pdb.lookup_procedure("gimp-image-save-as")
    if not proc:
        proc = pdb.lookup_procedure("gimp-xcf-save")
    if not proc:
        proc = pdb.lookup_procedure("file-xcf-save")
    ...
    if proc:
        ...
        print("Warning: Could not save XCF file - no XCF save procedure found")
except Exception as e:
    print(f"Warning: XCF save failed: {{e}}")

try:
    print(f"DEBUG: image before duplication: {{image}}, type: {{type(image)}}")
    print("DEBUG: Starting image duplication")
    proc = pdb.lookup_procedure("gimp-image-duplicate")
    if proc:
        print("DEBUG: gimp-image-duplicate procedure found")
        config = proc.create_config()
        config.set_property("image", image)
        print("DEBUG: Running gimp-image-duplicate...")
        flat_result = proc.run(config)
        print(f"DEBUG: flat_result type: {{type(flat_result)}}, value: {{flat_result}}")
        try:
            # Try to extract the image - it might be the result directly or at index 0
            flat_image = None
            print(f"DEBUG: hasattr get_image: {{hasattr(flat_result, 'get_image')}}")
            print(f"DEBUG: hasattr index: {{hasattr(flat_result, 'index')}}")
            if hasattr(flat_result, 'length'):
                print(f"DEBUG: flat_result.length(): {{flat_result.length()}}")
            print(f"DEBUG: dir(flat_result): {{dir(flat_result)}}")
            if hasattr(flat_result, 'get_image'):
                flat_image = flat_result.get_image()
                print(f"DEBUG: Got image directly from flat_result")
            elif hasattr(flat_result, 'index'):
                try:
                    # The first value is the status code, image is at index 1
                    val = flat_result.index(1)
                    print(f"DEBUG: val at index 1: {{val}}, type: {{type(val)}}")
                    if hasattr(val, 'get_image'):
                        flat_image = val.get_image()
                        print(f"DEBUG: Found image at index 1")
                except Exception as ex:
                    print(f"DEBUG: Exception accessing index: {{ex}}")
            print(f"DEBUG: flat_image extracted: {{flat_image}}")
        except (IndexError, AttributeError) as idx_err:
            print(f"DEBUG: Failed to extract image from result: {{idx_err}}")
            flat_image = None
    else:
        print("DEBUG: gimp-image-duplicate procedure not found!")
        flat_image = None
except Exception as e:
    print(f"Warning: Could not duplicate image: {{e}}")
    flat_image = None
...
if flat_image:
    try:
        ...
        print(f"Warning: Could not flatten image: {{e}}")
    ...
    try:
        ...
        print(f"Warning: Could not get active layer: {{e}}")
        flat_layer = None

if flat_image:
    try:
        print(f"DEBUG: Attempting to save PNG to {{'{escape(output_png)}'}}")
        proc = pdb.lookup_procedure("file-png-save")
        if not proc:
            print(f"DEBUG: file-png-save not found, trying gimp-file-save")
            proc = pdb.lookup_procedure("gimp-file-save")
        if not proc:
            print(f"DEBUG: gimp-file-save not found, trying gimp-image-export")
            proc = pdb.lookup_procedure("gimp-image-export")
        if not proc:
            print(f"DEBUG: gimp-image-export not found, trying gimp-image-export-as")
            proc = pdb.lookup_procedure("gimp-image-export-as")

        if proc:
            print(f"DEBUG: Found procedure, creating config")
            config = proc.create_config()
            config.set_property("run-mode", Gimp.RunMode.NONINTERACTIVE)
            config.set_property("image", flat_image)
            file_obj = Gio.File.new_for_path("{escape(output_png)}")
            ...
            try:
                ...
                print(f"DEBUG: Could not set compression property")
            print(f"DEBUG: Running save procedure")
            proc.run(config)
            print(f"DEBUG: PNG save completed")
        else:
            print(f"Warning: Could not find any PNG save procedure")
    except Exception as e:
        print(f"Warning: PNG save failed: {{e}}")
